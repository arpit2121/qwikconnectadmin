...
const authSlice = createSlice({
  ...
  initialState: {
    ...
    refreshToken: savedToken ? savedToken.refresToken : null
  },
  reducers: {},
  extraReducers: (builder) => {
    builder.addMatcher(
      ...
      (state, { payload }) => {
        const tokenData = jwtDecode(payload.access_token);
        state.token = payload
        state.accessToken = payload.access_token
        state.user = jwtDecode(payload.access_token);
        state.adminId = tokenData?.sub.split('|')[1];
        // state.adminId = payload.adminId
        state.emailId = tokenData?.user_email;
